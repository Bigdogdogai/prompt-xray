...
[Prompt X-Ray coverage notes](docs/coverage.md) 映射 15 个常见 prompt failure pattern，其中包含 3 个 injection-style 风险、2 个 hidden-reasoning leakage case、4 个 missing/weak output-format 问题，以及 1 个 ordinary-writing false-trigger check。它们是维护者编写的 worked examples 和 self-evaluation snapshot，不是第三方 benchmark，也不是跨模型质量声明。具体修复见 [before/after examples](docs/before-after.md)。
