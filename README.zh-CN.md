...
`prompt-xray` 是一个轻量 agent skill/reference，用于 **Prompt X-Ray**：创建、审核、改写、测试、比较 prompt，并把重复工作流打包成 AI coding agent 可用的 Markdown skill 或规则。
...
[Prompt X-Ray benchmark](docs/benchmark.md) 覆盖 15 个常见 prompt failure pattern，其中包含 3 个 injection-style 风险、2 个 hidden-reasoning leakage case、4 个 missing/weak output-format 问题，以及 1 个 ordinary-writing false-trigger check。它是透明的手动 scorecard，不是 hosted automated benchmark。具体修复见 [before/after examples](docs/before-after.md)。
...
有意延后的发布项见 [docs/launch-followups.md](docs/launch-followups.md)。
