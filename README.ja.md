...
同梱の [Prompt X-Ray coverage notes](docs/coverage.md) は 15 個の一般的な prompt failure pattern を扱います。3 個の injection-style risk、2 個の hidden-reasoning leakage case、4 個の missing / weak output-format specification、1 個の ordinary-writing false-trigger check を含みます。これは maintainer-authored worked examples と self-evaluation snapshot であり、third-party benchmark や model-comparable quality claim ではありません。具体的な修正例は [before/after examples](docs/before-after.md) を参照してください。
