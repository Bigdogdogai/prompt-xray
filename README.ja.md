...
**Prompt X-Ray** は、実用的な prompt engineering のための小さな agent skill/reference です。プロンプトの作成、監査、書き換え、テスト、比較、そして AI coding agent 向けの Markdown skill / rules 化を支援します。
...
同梱の [Prompt X-Ray coverage notes](docs/coverage.md) は 20 個の prompt failure pattern を扱います。direct injection、subtle few-shot / Markdown/HTML injection、hidden-reasoning leakage、weak output contract、local workflow risk、long-form realistic prompt、ordinary-writing false-trigger check を含みます。worked examples と coverage notes として使い、third-party benchmark や model-comparable quality claim として扱わないでください。具体的な修正例は [before/after examples](docs/before-after.md) を参照してください。
