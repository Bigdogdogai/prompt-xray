...
**Prompt X-Ray**는 실용적인 prompt engineering을 위한 작고 가벼운 agent skill/reference입니다. 프롬프트 생성, 감사, 재작성, 테스트, 비교, 그리고 AI coding agent용 Markdown skill/rules 패키징을 지원합니다.
...
포함된 [Prompt X-Ray coverage notes](docs/coverage.md)는 20개의 prompt failure pattern을 다룹니다. direct injection, subtle few-shot / Markdown/HTML injection, hidden-reasoning leakage, weak output contract, local workflow risk, long-form realistic prompt, ordinary-writing false-trigger check를 포함합니다. worked examples와 coverage notes로 사용하고, third-party benchmark 또는 model-comparable quality claim으로 취급하지 마세요. 구체적인 수정 예시는 [before/after examples](docs/before-after.md)를 참고하세요.
